...
from model.model_lr import Model
...
def base_build_dataset(name):
    return getattr(importlib.import_module('dataset.dataset_lr', package=None), name)()
...
def evaluate(model, val_data, name, nr_eval, step, save_img):
    ...
    if name == "CityValDataset" or name == "KittiValDataset" or name == "DavisValDataset":
        ...
        with torch.no_grad():
            lpips_score_avg, psnr_score_avg, msssim_score_avg, ssim_score_avg = np.zeros(5), np.zeros(5), np.zeros(5), np.zeros(5)
            for i, data in enumerate(val_data):
                start = time.time()
                data = data.to(device, non_blocking=True) / 255.    # (B, 7, C, H, W)
                preds = model.eval(data, name)                      # (B, 5, C, H, W)
                eval_time = time.time() - start
                logger.info(f'[eval] epoch: {nr_eval}, step: {i + 1} / {step_per_epoch}, time: {eval_time: .2f}s')
                b, num_pred, c, h, w = preds.shape
                assert b == 1 and num_pred == 5
                gt_imgs, pred_imgs = data[0], preds[0]  # (7, C, H, W), (5, C, H, W)
                for j in range(num_pred):
                    psnr_val = -10 * math.log10(torch.mean((gt_imgs[j + 2] - pred_imgs[j]) * (gt_imgs[j + 2] - pred_imgs[j])).cpu().data)
                    ssim_val = ssim(gt_imgs[j + 2: j + 3], pred_imgs[j: j + 1], data_range=1.0, size_average=False)
                    ms_ssim_val = ms_ssim(gt_imgs[j + 2: j + 3], pred_imgs[j: j + 1], data_range=1.0, size_average=False)
                    lpips_val = loss_fn_alex(
                        ((gt_imgs[j + 2: j + 3] - 0.5) * 2.0).clone(),
                        ((pred_imgs[j: j + 1] - 0.5) * 2.0).clone()
                    )
                    psnr_score_avg[j] += psnr_val
                    lpips_score_avg[j] += lpips_val
                    ssim_score_avg[j] += ssim_val
                    msssim_score_avg[j] += ms_ssim_val
            psnr_score_avg /= step_per_epoch
            ssim_score_avg /= step_per_epoch
            msssim_score_avg /= step_per_epoch
            lpips_score_avg /= step_per_epoch
            for i in range(5):
                logger.info(
                    f'dataset: {name}, epoch: {nr_eval}, ' +
                    f'psnr_{i}: {np.mean(psnr_score_avg[: (i + 1)]): .4f}, ' +
                    f'ssim_{i}: {np.mean(ssim_score_avg[: (i + 1)]): .4f}, ' +
                    f'ms_ssim_{i}: {np.mean(msssim_score_avg[: (i + 1)]): .4f}, ' +
                    f'lpips_{i}: {np.mean(lpips_score_avg[: (i + 1)]): .4f}'
                )
