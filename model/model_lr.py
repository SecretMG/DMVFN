...
class Model:
    ...
    def eval(self, imgs, name='city', scale_list = [4,4,4,2,2,2,1,1,1]):
        self.dmvfn.eval()
        b, n, c, h, w = imgs.shape  # (B, 7, C, H, W)
        preds = []
        if name == 'CityValDataset':
            ...
            img_0, img_1 = imgs[:, 0], imgs[:, 1]
            for t_pred in range(5):
                merged = self.dmvfn(torch.cat((img_0, img_1), 1), scale=scale_list, training=False)
                length = len(merged)
                if length == 0:
                    pred = img_0
                else:
                    pred = merged[-1]
                img_0, img_1 = img_1, pred
                preds.append(pred)
            assert len(preds) == 5
